...
# imports for cython 
import cython
from cython.cimports.libc.math import fabs, pow, exp, log, log1p, INFINITY
...
###############
@cython.cfunc
def get_fixed_point_case1(alpha: cython.double, 
                          upper: cython.double,
                          tol: cython.double = 1e-7):
    '''
...
@cython.cfunc
def _core_logic_case1(x: cython.double, 
                      upper: cython.double, 
                      alpha: cython.double, 
                      one_minus_alpha: cython.double, 
                      fixed_point: cython.double, 
                      tol: cython.double) -> cython.double:
    '''
    ...
    one_minus_x: cython.double = 1.0 - x
    c1: cython.double
    c2: cython.double
    expo: cython.double
    ...
    low: cython.double = 0.0
    high: cython.double = one_minus_x
    
    # M_low, M_high below assume low = 0 and high = 1 - x
    # if x <= fixed_point, expo > 0 and M_low is finite, if expo < 0, M_low is infinite
    M_low:  cython.double = c2 if x <= fixed_point else INFINITY

    #M_high: cython.double = c1 * pow(high, expo) + c2 * pow(1 - high, expo)
    M_high: cython.double = c1 * pow(high, expo) + c2 * pow(x, expo)
    
    # init all other vars for bisection
    mid: cython.double = 0.5 * (high - low)
    one_minus_mid: cython.double
    term1: cython.double
    term2: cython.double
    M_mid: cython.double
    grad: cython.double
    newton: cython.double
    secant: cython.double
...
@cython.cfunc
def get_FNR_case1_vec(x_array: cython.double[:], 
                    alpha: cython.double, 
                    rho: cython.double, 
                    tol: cython.double = 1e-7):

    # compute upper and one_minus_alpha once to save compute in core_logic
    log_upper: cython.double = (alpha - 1.0) * rho
    upper: cython.double = exp(log_upper)
    one_minus_alpha: cython.double = 1.0 - alpha

    # compute fixed point (hard code 1e-15 for stability)
    fixed_point: cython.double = get_fixed_point_case1(alpha, upper, tol = 1e-15)

    # allocate output array
    n: cython.Py_ssize_t = x_array.shape[0]
    result_np = np.empty(n, dtype=np.float64)
    result_view: cython.double[::1] = result_np
    
    # run core logic in C for loop
    i: cython.Py_ssize_t
    for i in range(n):
        result_view[i] = _core_logic_case1(x_array[i], upper, alpha, one_minus_alpha, fixed_point, tol)
...
###############
@cython.cfunc
def get_fixed_point_case2(rho: cython.double, 
                          tol: cython.double = 1e-7):
    '''
...
@cython.cfunc
def _core_logic_case2(x: cython.double,
                      rho: cython.double,
                      fixed_point: cython.double,
                      tol: cython.double) -> cython.double:
    '''
    ...
    # save variables to reduce computation of M for later
    one_minus_x: cython.double = 1.0 - x
    log_one_minus_x: cython.double = log1p(-x)
    log_x: cython.double = log(x)

    less_than_fixed_point: cython.bint = (x <= fixed_point)
    ...
    # begin bisection 
    mid: cython.double = 0.5 * (low + high)
    one_minus_mid: cython.double
    M_mid: cython.double
    grad: cython.double
    newton: cython.double
...
@cython.cfunc
def get_FNR_case2_vec(x_array: cython.double[:], 
                    rho: cython.double, 
                    tol: cython.double = 1e-7):
    
    # compute fixed point (hard code 1e-15 tol for stability)
    fixed_point: cython.double = get_fixed_point_case2(rho, tol = 1e-15)
    ...
    i: cython.Py_ssize_t
    for i in range(n):
        result_view[i] = _core_logic_case2(x_array[i], rho, fixed_point, tol)
...
###############
###############
def get_fixed_point_case3(alpha: cython.double, 
                          lower: cython.double,
                          x_max: cython.double,
                          tol: cython.double = 1e-7):
    '''
...
@cython.cfunc
def _core_logic_case3(x: cython.double, 
                      lower: cython.double, 
                      alpha: cython.double, 
                      one_minus_alpha: cython.double, 
                      fixed_point: cython.double, 
...
@cython.cfunc
def get_FNR_case3_vec(x_array: cython.double[:], 
                    alpha: cython.double, 
                    rho: cython.double, 
                    tol: cython.double = 1e-7):

    # compute lower, one_minus_alpha, x_max once to save compute in core_logic
    log_lower: cython.double = (alpha - 1.0) * rho
    lower: cython.double = exp(log_lower)
    one_minus_alpha: cython.double = 1.0 - alpha
    x_max: cython.double = exp( (alpha-1)/alpha * rho)

    # compute fixed point (hard code 1e-15 for stability)
    fixed_point: cython.double = get_fixed_point_case3(alpha, lower, x_max, tol = 1e-15)
    ...
    i: cython.Py_ssize_t
    for i in range(n):
        result_view[i] = _core_logic_case3(x_array[i], lower, alpha, one_minus_alpha, fixed_point, x_max, tol)
